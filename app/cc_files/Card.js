...
var Card = Class.create({
    ...
    //console.log(apduByteString.toString(HEX));
		...
		if (typeof wrapScript == "undefined") {
			promise = this.exchange(apduByteString, le);
		}
		else {
			promise = wrapScript.exchangeWrapped(apduByteString, le);
		}
    //console.log(this.response.toString(HEX) + Convert.toHexShort(this.SW));
    	return promise.then(function(response) {
    		currentObject.response = response;
			if (swCheck) {
				for (var i=0; i<swCheck.length; i++) {
					if (typeof swCheck[i] == "number") {
						if (currentObject.SW == swCheck[i]) {
							break;
						}
					}
					else {
						if ((currentObject.SW & swCheck[i][1]) == swCheck[i][0]) {
							break;
						}
					}
				}
				if (i == swCheck.length) {
					throw "Invalid status " + i + " - " + Convert.toHexShort(currentObject.SW);
				}			
			}		
